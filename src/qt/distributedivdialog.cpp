...
class QReservedItem : public QStandardItem
{
public:
    QReservedItem()
    {
        QVariant reservedString("Reserved Share");
        setData(reservedString, Qt::DisplayRole);
        setData(reservedString, Qt::UserRole);
    }
};
...
{
    fSuccess = false;
    sError = tr("Scanning thread did not terminate properly").toStdString();
    try
    {
...
void BalanceScannerThread::Scan(unsigned int cutoffTime)
{
    ...
    progressDialog.setLabelText(tr("Scanning local blockchain"));
...
const QString DistributeDivDialog::columnHeadings[] = {
    tr("YBCoin Address"), tr("Shares"), tr("Bitcoin Address"), tr("Dividend")
};
...
void DistributeDivDialog::on_calcDividendsButton_clicked()
{
    ...
    for (DistributionVector::const_iterator it = vDistribution.begin(); it != vDistribution.end(); it++, i++)
    {
        ...
        if(sReservedShares == it->GetPeershareAddress().ToString())
            model->setItem(i, 2, new QReservedItem());
        else
...
void DistributeDivDialog::on_exportButton_clicked()
{
    ...
    fprintf(fp,"%s,%s,%s,%s\n", columnHeadings[0].toStdString().c_str(), columnHeadings[1].toStdString().c_str(), columnHeadings[2].toStdString().c_str(), columnHeadings[3].toStdString().c_str());
